...
ONLINE_LEADS = {"남성": "유다휘", "여성": "정지형", "키즈": "이지현", "글로벌": "신명철"}  # ★2026-09-10 우먼 담당 교체(한상은 육아휴직 → 정지형, 둘 다 '무탠다드 우먼 온라인MD')
...
def load_26fw_prep(sheets, sid=None):
    """→ {품번: {'t','o','f', 'colors': {신품번+컬러: {'t','o','f'}}}} — 준비물량 AA/AB/AC.

    ★시점재고(AS)·발주량 분해는 2026-08-06에 넣었다가 사용자 지시로 되돌렸다 —
      화면엔 준비수량(AA) 하나만 쓴다. 대신 SKU(컬러) 단위 내역을 colors로 함께 준다
      (STY 상세의 '컬러별 준비수량').
    """
    def _n(v):
        try:
    ...
    for r in vals:
        ...
        if not (t or o or f):
            continue
        d = out.setdefault(sty, {"t": 0, "o": 0, "f": 0, "colors": {}})
        d["t"] += round(t); d["o"] += round(o); d["f"] += round(f)
        cd = d["colors"].setdefault(code, {"t": 0, "o": 0, "f": 0})
        cd["t"] += round(t); cd["o"] += round(o); cd["f"] += round(f)
    return out
...
# ★2026-08-07 개명 추적: "26년 캠페인_특별 기획전" → "26년 스페셜이슈".
#   구 이름 탭이 사라져 400 이 났다. 같은 시트의 이 탭이 **B5:L 헤더가 완전히 동일**
...
# ★2026-08-07 개명 추적: "26년 프로모션 경로" → "26년 일반 기획전 경로".
#   20컬럼 레이아웃 동일(G형태 H시작일 M제목 R담당자), 헤더 7행·데이터 8행도 그대로 →
#   기존 range(A8:T400) 유지. G열 형태 분포 = 일반 기획전 311 / 특별 기획전 11 / CPCMS 8 /
#   쇼케이스 2 / 에디션 1 이라 아래 '일반 기획전' 필터가 그대로 유효(시작일 有 307건).
PROMO_TAB = "26년 일반 기획전 경로"
