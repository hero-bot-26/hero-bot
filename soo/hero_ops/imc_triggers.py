...
def main() -> int:
    ...
    groups = compute_imc(releases, as_of)
    msgs = format_imc(groups)
    n = sum(len(v) for v in groups.values())
    print(f"기준일 {as_of} · 발매 {len(releases)}건 · 노출알람 그룹 {len(msgs)} (발매 {n}건) · TEST_ONLY={T.TEST_ONLY}")
    for m in msgs:
        print("\n" + "─" * 50)
