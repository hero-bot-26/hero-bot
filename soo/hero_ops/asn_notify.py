...
# ★2026-09-10 True — 실담당자 발송 개시(사용자 지시 "오늘부터 담당자 나가게").
#   그 전 이력은 전부 TEST(본인DM)였다. 원장 전수 확인 결과 9/9 164건·9/10 21건 모두
#   `TEST→본인DM` 라벨이고 **LIVE 라벨은 0건** — 9/9 에 스위치를 켰다고 기록했지만
...
#   ★팀장 심혁은 **세 라인 모두**에 들어간다(2026-09-17 사용자 지시 — 김병관 처럼 전 건을 본다).
#     ME 는 상품컨트롤 대상이 아니라는 결정이 그대로라 팀장도 ME 는 안 받는다.
CTRL_LEADS = {
    "MM": ["김성은", "박기홍", "최종민", "박대영", "두현진", "이수민", "심혁"],
    "MW": ["김성은", "정소이", "최종민", "박대영", "두현진", "이수민", "심혁"],
    "MK": ["김성은", "최종민", "박대영", "두현진", "이수민", "심혁"],
}
...
#   ★★**디자이너·소싱 팀장은 붙이지 않는다**(사용자 결정 2026-09-11). `담당자매핑` 에
#     `ds` 팀장 김지훈·임혜진·`sc` 팀장 윤정민이 Slack ID 까지 채워져 있어 붙이기 쉬워 보이지만
#     물은 것이고 답은 "붙이지 말자"였다. 참고로 ds 팀장은 둔 분 다 `category=전체` 라
#     나눌 축이 없고, 붙였으면 두 분이 각자 전 건을 받는다.
MD_LEADS = {
