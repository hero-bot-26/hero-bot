...
# COMMAND ----------
  ...
  stocks AS (
    SELECT a.dt,
      ...
      a.`재고수량` AS qty,
      CAST(b.normal_price AS BIGINT) * CAST(a.`재고수량` AS BIGINT) AS normal_price_amt,
      CAST(b.wonga AS BIGINT) * CAST(a.`재고수량` AS BIGINT) AS wonga_amt
...
# ★2026-08-07 경량화 — goods 단위 → **base 품번(style) 단위** 집계.
#   소비처 둘 다 base 품번만 쓴다: 생성기 _hero_of_fw 는 style.split('-')[0] 로 히어로를 찾고,
#   weekly_review 도 x[2].split('-')[0] 로 STY 를 맞춘다. m.style_no 는 '품번-컴러'(SKU)라
#   goods 와 거의 1:1(1,005종)이었고, base 로 접으면 164종이 된다.
#   실측: 156,142행 → 25,207행(84%↓). 창(시즌 2/1~)은 그대로 유지.
#   ★컴럼 순서(date, goods_no, style_no, pdp_uv)는 절대 바꾸지 말 것 —
#     weekly_review 가 위치(x[2]·x[3])로 읽는다.
#   style_no 가 비면 goods_no 를 키로 써서 생성기의 goods 폴백 경로를 살린다.
...
"""
insert_query_result("PDP일별", spark.sql(pdp_daily_query), label=f"히어로 품번 x 일별 PDP 유입 UV(direct) {_pdpd_start}~{date}")
print("[OK] PDP일별 기록 완료")

# COMMAND ----------


# (5) 오프라인 매장별 — 히어로 x 매장 매출 + 매장 총계. 성과탭 '오프라인' 블록 소스.
#     ★2026-08-07 신설. 기존 매출 셀은 `GROUP BY channel, goods_no, goods_opt, brand` 로
#       **매장 차원을 버리고** 있었고 v_goods_filter(히어로) 로만 뽑아 **비중의 분모(전체)도 없었다**.
#     두 탭으로 나눈다:
#       · 오프라인매장별   = 히어로 uid x 매장 x base품번 → 매장 랭킹·침투지수의 분자
#       · 오프라인매장총계 = 필터 없음(무탠 4브랜드 전 상품) x 매장 → 비중·지수의 분모
#     ★매장 정의와 GMV 산식은 매출 셀(offline_processed)과 **똑같이** 간다. 어긋나면 실적과 안 맞는다:
#       v_shop_list(offline+selectshop+shop_no=68) · gmv = IF(sales_type='SALE',1,-1)*sales_price
#       (사용자 확정 2026-08-07: 셀렉샵 전부 포함. 무신사 스토어처럼 무탠 취급이 작은 곳은
#        침투지수가 크게 튀지만 랭킹 기본 정렬이 매출순이라 아래에 깔린다.)
#     기간·전년 창은 PMKT과 동일(_pmkt_periods: YTD/MTD/WEEK/FWTD).
_OFF_BRANDS = "('musinsastandard','musinsastandardhome','musinsastandardwoman','musinsastandardkids')"


def _off_query(hero_only):
    """hero_only=True → 히어로 x 매장 x 품번(분자) / False → 매장 총계(분모)."""
    parts = []
    for nm, s, e, sly, ely in _pmkt_periods:
        cur = f"DATE(pos.sales_date) BETWEEN TO_DATE('{s}','yyyyMMdd') AND TO_DATE('{e}','yyyyMMdd')"
        ly = f"DATE(pos.sales_date) BETWEEN TO_DATE('{sly}','yyyyMMdd') AND TO_DATE('{ely}','yyyyMMdd')"
        gfj = "  JOIN v_goods_filter gf ON pos.goods_no = gf.goods_no" if hero_only else ""
        mj = "  LEFT JOIN v_meta m ON CAST(pos.goods_no AS STRING) = CAST(m.goods_no AS STRING)" if hero_only else ""
  ...
  SELECT '{nm}' AS period, CAST(pos.shop_no AS STRING) AS shop_no,
         ...
         SUM(CASE WHEN {ly} THEN IF(pos.sales_type='SALE',1,-1)*pos.sales_price ELSE 0 END) AS gmv_ly,
         SUM(CASE WHEN {ly} THEN IF(pos.sales_type='SALE',1,-1)*pos.qty ELSE 0 END) AS qty_ly
  FROM musinsa.order_group.pos_order_sales pos
  JOIN v_shop_list sl ON pos.shop_no = sl.shop_no
  JOIN musinsa.order_group.shop sh ON pos.shop_no = sh.shop_no
{gfj}
{mj}
  WHERE LOWER(pos.brand_id) IN {_OFF_BRANDS}
    AND (({cur}) OR ({ly}))
  GROUP BY CAST(pos.shop_no AS STRING), {sty}""")
    return "SELECT * FROM (" + " UNION ALL ".join(parts) + ") t WHERE gmv <> 0 OR gmv_ly <> 0"


insert_query_result("오프라인매장별", spark.sql(_off_query(True)),
                    label=f"히어로 품번 x 매장 오프라인 매출(POS) ~{date}")
insert_query_result("오프라인매장총계", spark.sql(_off_query(False)),
                    label=f"매장 오프라인 매출 총계(무탠 전 상품, 비중 분모) ~{date}")
print("[OK] 오프라인매장별 / 오프라인매장총계 기록 완료")
