...
insert_query_result("PMKT기간", spark.sql(_pmkt_union), label=f"히어로 goods x 기간(YTD/MTD/WEEK) PMKT(direct) ~{date}")

# (2) 주차 추세 — goods x ISO주차 (거래액/PDP 스파크라인용)
pmkt_week_query = f"""
WITH base AS (
  SELECT CAST(p.goods_no AS STRING) AS goods_no, m.style_no AS style_no,
         TO_DATE(p.dt,'yyyyMMdd') AS d, p.pdp_uv_cnt, p.purchase_uv_cnt, p.qty, p.gmv, p.prev_path1
  {_PMKT_FROM.rstrip()}
    AND p.dt BETWEEN '20260101' AND '{date}'
)
...
"""
insert_query_result("PMKT주차", spark.sql(pmkt_week_query), label=f"히어로 goods x 주차 PMKT 퍼널(direct) 2026~{date}")
